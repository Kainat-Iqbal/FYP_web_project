...
const viewResultApprovalDean = async (req, res) => {
...
WHERE
  s.approvedHod = 'Yes' AND
  s.approvedDean IS NULL;
`;
  // Execute the query
...
const viewSelectedResultDean = async (req, res) => {
...
const approveResultDean = async (req, res) => {
  const { id } = req.params;
  const {deanId} = req.body;

  const queryToUpdateApproveRequest =
    "UPDATE `status` SET `deanId`=?, `approvedDean`=? WHERE assignId = ?";
...
module.exports = { viewResultApprovalDean,viewSelectedResultDean,approveResultDean };
