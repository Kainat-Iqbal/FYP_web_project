...
const Proforma = () => {
    ...
    const fetchStudentData = async () => {
      try {
        const response = await axios.get(
          `http://192.168.100.18:8081/result/GetIndividualStudentResult/${userId}`,
          {
  ...
  useEffect(() => {
      const fetchDataofLastSemester = async () => {
          try {
              const response = await axios.get(`http://192.168.100.18:8081/result/GetIndividualStudentLastSemesterResult/${userId}`, {
                  withCredentials: true,
              });
