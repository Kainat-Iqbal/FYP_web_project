...
const Insights = () => {
  ...
  useEffect(() => {
    const fetchStudentDetails = async () => {
      try {
        const response = await axios.get(`http://192.168.100.18:8081/student/ViewIndividualStudentDetail/${userId}`);
    ...
    const fetchCgpaDistribution = async () => {
      try {
        const response = await axios.get(`http://192.168.100.18:8081/student/ViewStudentDetailWRTSesmester/${userId}`);
